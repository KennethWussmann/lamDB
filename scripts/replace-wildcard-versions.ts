...
const replaceWildcards = (dependencies: Record<string, string>, versions: Record<string, string>): Record<string, string> =>
  Object.fromEntries(
...
(async () => {
  ...
  const currentVersions: Record<string, string> = Object.fromEntries(Object.values(packageJsons).map((pkg) => [pkg.name, pkg.version]));
  await Promise.all(
